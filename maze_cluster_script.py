...
moving_source_production_strength = 1.0 # 1.0 #strength of the source
...
parameter_dict={'Dc': Dc, 'Dp': Dp, 'Bp': Bp, 'moving_source_production_strength': moving_source_production_strength, 'sp_decay_rate': sp_decay_rate,
                ...
                'birth_steps': birth_steps, 'active_mask': active_mask, 'dead_tracker': dead_tracker, 'death_zone_map': death_zone_map, 'emitter_position': tuple(emitter_position),
...
with open(filename2, 'a') as f:
    for particle_id in range(num_particles):
        f.write(f"{max(exit_times[particle_id]-birth_steps[particle_id] *dt, -1)} {beta} {job_id} {particle_id}\n")
